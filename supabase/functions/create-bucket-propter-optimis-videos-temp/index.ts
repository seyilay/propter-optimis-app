...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'propter-optimis-videos',
        name: 'propter-optimis-videos',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["video/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for propter-optimis-videos" ON storage.objects FOR SELECT USING (bucket_id = 'propter-optimis-videos');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for propter-optimis-videos" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'propter-optimis-videos');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for propter-optimis-videos" ON storage.objects FOR UPDATE USING (bucket_id = 'propter-optimis-videos');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for propter-optimis-videos" ON storage.objects FOR DELETE USING (bucket_id = 'propter-optimis-videos');`
    ];
